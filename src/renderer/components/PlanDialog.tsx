...
import { ReferenceUnderstanding } from './VisualReferences';
import { VideoUnderstanding } from './VideoReferences';
  ...
  return (
    <dialog className="plan-dialog" ref={dialog} aria-labelledby="plan-title" onCancel={e => { e.preventDefault(); if (!busy) onClose(draft, attachedFiles); }}>
      ...
      <div className="plan-scroll">
        ...
        <p className="plan-analysis-note">生成方案会调用当前模型，计入分析消耗；此时不生成游戏代码或素材。{draft.attachmentCount > 0 ? ` 本次 ${draft.attachmentCount} 个附件将在开始制作时导入，生产附件在制作时使用；游戏参考图通过独立入口参与看图规划。关闭应用后需重新添加生产附件。` : ''}</p>
        ...
        {!!draft.references?.length && !draft.run && <ReferenceUnderstanding key={draft.version?.id ?? draft.id} draft={draft} disabled={busy || generating || editingId !== null || videoEditing} onEditing={setReferenceEditing} onSaved={next => { setDraft(next); setSelected(null); }} />}
        {!!draft.video && !draft.run && <VideoUnderstanding key={`video-${draft.version?.id ?? draft.id}`} draft={draft} disabled={busy || generating || editingId !== null || referenceEditing} onEditing={setVideoEditing} onSaved={next => { setDraft(next); setSelected(null); }} />}
        ...
        {!generating && !draft.run && draft.version && <>
          ...
          <fieldset className="plan-options"><legend>比较并选择一套方案</legend>
            {draft.version.options.map((option, index) => <article key={option.id} className={`plan-option${selected === option.id ? ' is-selected' : ''}${editingId === option.id ? ' is-editing' : ''}`}>
              <label className="plan-option-select"><input type="radio" name="game-plan" value={option.id} checked={selected === option.id} disabled={busy || editingId !== null || referenceEditing || videoEditing} onChange={() => setSelected(option.id)} /><span className="plan-number">0{index + 1}</span><h3>{option.title}</h3></label>
              ...
              {editingId === option.id ? <PlanEditor draft={draft} option={option} onCancel={() => setEditingId(null)} onSaved={next => { setDraft(next); setEditingId(null); setSelected(null); }} /> : <button type="button" disabled={busy || editingId !== null || referenceEditing || videoEditing} onClick={() => setEditingId(option.id)}>编辑此方案</button>}
          ...
          {!editingId && !referenceEditing && !videoEditing && <PlanRevisionEditor key={draft.version.id} draft={draft} busy={busy} onRevise={input => action(async () => { setSelected(null); setDraft(await window.noobi.revisePlans({ draftId: draft.id, versionId: draft.version!.id, ...input })); })} />}
          <p className="plan-version">已保存 · 第 {draft.version.number} 版 · {draft.version.reusedAnalysis ? '复用相同条件的已保存分析 · 本次未调用模型' : draft.version.authoredBy === 'user' ? '你手动编辑 · 未调用模型' : <>分析用时 {Math.ceil(draft.version.analysisDurationMs / 1000)} 秒 · {draft.version.model ?? '默认模型'} · {draft.version.analysisUsage ? `${draft.version.analysisUsage.totalTokens} tokens` : '模型未上报 token 用量'}</>}</p>
      ...
      <footer className="plan-footer">
        ...
        <div className="plan-actions">
          {generating ? <button type="button" disabled={busy} onClick={() => void action(async () => setDraft(await window.noobi.cancelPlan(draft.id)))}>取消生成</button> : !draft.run ? <button type="button" disabled={busy || editingId !== null || referenceEditing || videoEditing} onClick={() => void action(async () => { setSelected(null); setDraft(await window.noobi.retryPlan(draft.id)); })}>{draft.version?.requiresReview ? '校验编辑后的方案' : '重新生成'}</button> : null}
          {draft.run && draft.run.status !== 'dispatched' && <button type="button" disabled={busy} onClick={() => void action(async () => {
            setSelected(null); setDraft(await window.noobi.generatePlans({ request: draft.request, projectId: draft.run?.projectId ?? draft.projectId, model: draft.model, effort: draft.effort, attachmentCount: draft.run?.projectId ? 0 : draft.attachmentCount, references: draft.references, referenceSpecOverride: draft.referenceSpecOverride, video: draft.video, videoSpecOverride: draft.videoSpecOverride }));
          })}>重新规划</button>}
          <button type="button" className="plan-start" disabled={!ready || draft.version?.requiresReview || !selected || busy || editingId !== null || referenceEditing || videoEditing} onClick={() => void start()}>{busy ? <LoaderCircle size={16} className="spin" /> : <ChevronRight size={16} />} {busy ? '正在启动…' : '按此方案开始制作'}</button>
        </div>
      </footer>
    </dialog>
  );
