  ...
  return (
    <dialog className="plan-dialog" ref={dialog} aria-labelledby="plan-title" onCancel={e => { e.preventDefault(); if (!busy) onClose(draft, attachedFiles); }}>
      ...
      <div className="plan-scroll">
        ...
        {!!draft.references?.length && !draft.run && <ReferenceUnderstanding key={draft.version?.id ?? draft.id} draft={draft} disabled={busy || generating || editingId !== null} onEditing={setReferenceEditing} onSaved={next => { setDraft(next); setSelected(null); }} />}
        ...
        {!generating && !draft.run && draft.version && <>
          ...
          <fieldset className="plan-options"><legend>比较并选择一套方案</legend>
            {draft.version.options.map((option, index) => <article key={option.id} className={`plan-option${selected === option.id ? ' is-selected' : ''}${editingId === option.id ? ' is-editing' : ''}`}>
              <label className="plan-option-select"><input type="radio" name="game-plan" value={option.id} checked={selected === option.id} disabled={busy || editingId !== null || referenceEditing} onChange={() => setSelected(option.id)} /><span className="plan-number">0{index + 1}</span><h3>{option.title}</h3></label>
              ...
              {editingId === option.id ? <PlanEditor draft={draft} option={option} onCancel={() => setEditingId(null)} onSaved={next => { setDraft(next); setEditingId(null); setSelected(null); }} /> : <button type="button" disabled={busy || editingId !== null || referenceEditing} onClick={() => setEditingId(option.id)}>编辑此方案</button>}
          ...
          {!editingId && !referenceEditing && <PlanRevisionEditor key={draft.version.id} draft={draft} busy={busy} onRevise={input => action(async () => { setSelected(null); setDraft(await window.noobi.revisePlans({ draftId: draft.id, versionId: draft.version!.id, ...input })); })} />}
      ...
      <footer className="plan-footer">
        ...
        <div className="plan-actions">
          {generating ? <button type="button" disabled={busy} onClick={() => void action(async () => setDraft(await window.noobi.cancelPlan(draft.id)))}>取消生成</button> : !draft.run ? <button type="button" disabled={busy || editingId !== null || referenceEditing} onClick={() => void action(async () => { setSelected(null); setDraft(await window.noobi.retryPlan(draft.id)); })}>{draft.version?.requiresReview ? '校验编辑后的方案' : '重新生成'}</button> : null}
          ...
          <button type="button" className="plan-start" disabled={!ready || draft.version?.requiresReview || !selected || busy || editingId !== null || referenceEditing} onClick={() => void start()}>{busy ? <LoaderCircle size={16} className="spin" /> : <ChevronRight size={16} />} {busy ? '正在启动…' : '按此方案开始制作'}</button>
        </div>
      </footer>
    </dialog>
  );
