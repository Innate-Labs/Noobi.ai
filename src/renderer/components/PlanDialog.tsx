    ...
    <dialog className="plan-dialog" ref={dialog} aria-labelledby="plan-title" onCancel={e => { e.preventDefault(); if (!busy) onClose(draft, attachedFiles); }}>
      ...
      <div className="plan-scroll">
        ...
        {ready && draft.version && <>
          ...
          <fieldset className="plan-options"><legend>比较并选择一套方案</legend>
            {draft.version.options.map((option, index) => <article key={option.id} className={`plan-option${selected === option.id ? ' is-selected' : ''}`}>
              <label className="plan-option-select"><input type="radio" name="game-plan" value={option.id} checked={selected === option.id} disabled={busy} onChange={() => setSelected(option.id)} /><span className="plan-number">0{index + 1}</span><h3>{option.title}</h3></label>
          ...
          <p className="plan-version">已保存 · 第 {draft.version.number} 版 · 分析用时 {Math.ceil(draft.version.analysisDurationMs / 1000)} 秒 · {draft.version.model ?? '默认模型'} · {draft.version.analysisUsage ? `${draft.version.analysisUsage.totalTokens} tokens` : '模型未上报 token 用量'}</p>
        </>}
      ...
      <footer className="plan-footer">
        ...
        <div className="plan-actions">
          {generating ? <button type="button" disabled={busy} onClick={() => void action(async () => setDraft(await window.noobi.cancelPlan(draft.id)))}>取消生成</button> : !draft.run ? <button type="button" disabled={busy} onClick={() => void action(async () => { setSelected(null); setDraft(await window.noobi.retryPlan(draft.id)); })}>重新生成</button> : null}
          ...
          <button type="button" className="plan-start" disabled={!ready || !selected || busy} onClick={() => void start()}>{busy ? <LoaderCircle size={16} className="spin" /> : <ChevronRight size={16} />} {busy ? '正在启动…' : '按此方案开始制作'}</button>
        </div>
      </footer>
    </dialog>
  );
