...
describe('ExperienceReport', () => {
  it('keeps the collapsed check-count summary in the preview toolbar', () => {
  ...
  it('keeps existing reports available for stopped and running projects', () => {
    ...
    expect(shouldShowExperienceReport(false, 'failed', null)).toBe(true);
    expect(shouldShowExperienceReport(false, 'waiting', null)).toBe(true);
  });

  it('excludes skipped checks from the basic-check total', () => {
    const markup = renderToStaticMarkup(createElement(ExperienceReportTrigger, {
      report: { ...report, checks: [
        ...report.checks,
        { ...report.checks[0]!, id: 'input-response', status: 'repair' },
        { ...report.checks[0]!, id: 'restart', status: 'skipped' },
      ] },
      projectStatus: 'failed', evaluating: false, expanded: false, onToggle: vi.fn(),
    }));
    expect(markup).toContain('1/2 项');
    expect(markup).not.toContain('100');
  });
