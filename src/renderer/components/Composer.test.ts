...
describe('Composer', () => {
  ...
  it('keeps connection recovery visible while allowing the next instruction', () => {
    const markup = renderToStaticMarkup(createElement(Composer, {
      project: { ...project, lastError: '连接中断，正在重试' },
      models, settings, imageGenerationAvailable: true, onRun: vi.fn(), onStop: vi.fn(),
    }));
    expect(markup).toContain('role="status"');
    expect(markup).toContain('连接中断，正在重试');
    expect(markup.match(/<textarea[^>]*disabled/gu)).toBeNull();
  });
