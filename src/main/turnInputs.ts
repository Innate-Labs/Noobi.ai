...
export function turnInputs(prompt: string, imagePaths: readonly string[] = [], skills: Array<{ name: string; path: string }> = []): TurnInput[] {
  if (!Array.isArray(imagePaths) || imagePaths.length > 10 || imagePaths.some(path => typeof path !== 'string' || !isAbsolute(path) || path.includes('\0'))) throw new Error('模型图像输入路径无效或超过 10 张');
