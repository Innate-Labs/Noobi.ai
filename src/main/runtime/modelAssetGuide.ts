...
The host loads GLB in an independent renderer, measures dimensions/nodes/materials/decoded textures, checks named pivot/socket positions, and plays required clips at multiple timestamps to detect static or unbound tracks. animation.mode is none (required=[]), transform (actual transform clips), or skeletal (exported skin AND clips). Tool animation=true requires skeletal data. Rigid part animation is valid when explicitly selected, but is not skinning. Bind actual imported clips in Godot, verify foot contact, sockets, collision/occlusion and gameplay feedback. Sampling moving tracks is not proof of convincing motion or all poses.
