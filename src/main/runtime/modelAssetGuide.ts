...
Godot portability: do not animate translation/rotation/scale directly on a SkinnedMesh node. The importer can drop these channels even though Three.js plays them. The host rejects such exported targets. For whole-object motion, animate a named ordinary Group that contains both the mesh and its skeleton; for a skeletal pose animate weighted bones. Do not silently relabel rejected skeletal actions as transform animations. Reimport and inspect actual Godot tracks and vertex motion; importer resampling/optimization can alter peaks. In gameplay, let the movement controller own world translation unless the approved design explicitly requires root motion.
