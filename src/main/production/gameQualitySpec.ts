...
export function qualitySpecPrompt(spec: GameQualitySpec): string {
  return `Host quality specification ${spec.id}\nGenre: ${spec.genre}\n`
    ...
    + 'For the optional interaction/defeat adventure assembly profile, use GAME_ASSEMBLY_V1.md and data/game-assembly.json to connect real scene nodes, shared progression, UI, region checkpoints and the bound ArtBible palette. Host wiring checks never certify geometry, art or game completeness. '
