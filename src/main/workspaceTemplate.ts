...
export const NOOBI_HOST_RUNTIME_POLICY_VERSION = 8;
...
function workspaceFiles(project: WorkspaceProject): Record<string, string> {
  ...
  if (engine === 'godot') {
    return {
      ...
      'runtime/noobi/audio_v1.gd': GAME_AUDIO_KIT,
      'runtime/noobi/GAME_AUDIO_V1.md': GAME_AUDIO_GUIDE,
...
- For 2D/2.5D \`generate\`, call \`noobi_image_generate\` and follow its Codex ImageGen fallback when needed; lock subject design, style, palette, scale, frame dimensions, anchor, and view/camera angle across keyframes or a sprite sheet. For actual 3D, always call \`noobi_model3d_generate\`: the default route exports an image-guided Three.js model from referenceImage and sourcePath; only explicitly selected configured-api mode calls a 3D API. With \`animation=true\`, use and play a real rigged GLB clip; generated reference art cannot substitute for or prove a 3D clip.
...
- For every requested 3D asset, call \`noobi_model3d_generate\`. The default route requires a viewed reference image and AI-authored Three.js source, with multi-angle review before delivery. Only explicitly selected configured-api mode calls a 3D API; both routes return a validated, self-contained GLB 2.0 under \`public/assets/models/\`. ${project.engine === 'godot' ? 'Instantiate the returned GLB with a `res://public/assets/models/...` path in Godot. Three.js is build-time asset authoring only and must not run beside Godot.' : 'Load the returned GLB from production code rather than rebuilding an unrelated placeholder.'} Do not invent provider metadata or describe the low-poly fallback as high-fidelity API output.
...
### Image-guided Three.js model contract

- Default 3D route: generate/import and VIEW a single-object reference image, write model-sources/<name>.spec.json and an image-specific model-sources/<name>.mjs factory, then call noobi_model3d_generate with referenceImage and sourcePath. This overrides older API-first and six-preset instructions below. Explicit configured-api mode is a user choice only.
- Export async createModel(THREE, {referenceUrl}) returning {root, animations}. The host supplies Three.js in a sandbox without Node or external network, exports GLB and independently reloads it for front/side/back captures. Read the returned evidencePath; compare silhouette, proportions, materials and named critical features. Preserve editable source and inferred-backside notes.
- Technical export is not visual approval. The independent Reviewer must inspect the reference, all captures and production usage; wrong appearance, stale evidence, generic placeholders or missing animation requires repair. Correct at most 3 times and then report unresolved defects. Do not edit artifacts/model3d evidence. Godot remains the game runtime.
