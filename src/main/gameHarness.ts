...
import { assertRepairExecution } from './production/modelExecutionFailure.js';
...
export class GameHarness extends EventEmitter {
  ...
  async run(options: GameHarnessRunOptions): Promise<GameHarnessResult> {
    ...
    const prepareRepair = async (stage: string, findings: readonly string[]) => {
      ...
      assertRepairExecution(findings);
...
For ordinary environment props whose low-poly style genuinely fits the plan, use noobi_model3d_library_list then noobi_model3d_library_import to reuse a bundled CC0 asset. Inspect scale, materials and collision in the actual game. Library props are imported third-party art, not generated or reference-matched; never substitute them for a requested unique character or scene identity. Keep their license/source metadata.
...
2. Read runtime/noobi/MODEL_ASSETS_V2.md when present. New assets should use version 2 with the shared .noobi/art-bible.json, explicit rest dimensions, named pivot/sockets, collision intent and required animation clips; legacy specs remain assembly-unverified. Write model-sources/<name>.spec.json before code with {referenceImage, parts:[{name,shape,material}], criticalFeatures:[string], inferredSurfaces:[string]}. Include observable silhouette/proportions, named components and materials, 3–5 identity-defining features, pivots/sockets, dimensions, required animation, and explicitly inferred hidden surfaces. No fabricated visual scores.
...
For a finished 3D scene, inventory the full visible scene within the requested scope, including terrain, vegetation, rocks, architecture, player and enemies, not only interactive GLB assets. Source-code primitives and legacy fallback meshes also count as visible subjects. A few detailed props cannot cover an otherwise unfinished scene. Review gameplay-camera screenshots for coherent silhouette, materials, scale, lighting and readable HUD; declared placeholders remain unfinished. A focused iteration may retain out-of-scope placeholders only with an explicit remaining-gap record and must not claim whole-game completion.
