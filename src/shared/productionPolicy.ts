...
export type ProductionFailureCategory = 'network' | 'account' | 'provider' | 'build' | 'quality' | 'timeout' | 'budget' | 'no-progress' | 'interrupted' | 'unknown';
...
export const FAILURE_LABELS: Record<ProductionFailureCategory, string> = {
  network: '网络连接', account: '账户或配置', provider: '素材服务', build: '编译或构建', quality: '玩法或画面检查',
