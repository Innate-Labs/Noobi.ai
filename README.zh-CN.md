...
新工程还提供 `MODEL_ASSETS_V2.md`：统一风格规格以及尺寸、枢轴/挂点、碰撞意图和动画要求。宿主独立重载 GLB 后测量并检查采样顶点与加权蒙皮是否实际变化，空节点和无权重骨骼不算有效动作；真实碰撞、朝向、风格和游戏内动画仍需运行与画面验收。模型输入与绑定证据完全一致时才复用结果；旧资产不自动标为装配合格。
