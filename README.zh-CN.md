...
默认使用内置 CC0 素材库：2 首背景音乐、16 个音效，离线选取并导入，不调用音频 API。素材保留作者、来源和授权信息。这是现成素材选取，暂不提供定制配乐或语音；需要云端生成时，可在“设置 → 媒体 API”手动切换音频来源并保存。参见[素材来源与授权](resources/free-audio/LICENSES.md)。新 Godot 工程还提供音乐循环、双声部切换渐变、有限音效池与暂停/重开清理组件，复用游戏 UI 的分轨总线。原生工程混音验证通过；浏览器出声、生成成品接入与实际试听分别待验收。见[运行组件证据](docs/development-plan/reports/07-audio-runtime.md)。
