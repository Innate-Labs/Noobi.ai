...
新工程还提供 `MODEL_ASSETS_V2.md`：统一风格规格以及尺寸、枢轴/挂点、碰撞意图和动画要求。宿主独立重载 GLB 后测量并检查采样顶点与加权蒙皮是否实际变化，空节点和无权重骨骼不算有效动作；真实碰撞、朝向、风格和游戏内动画仍需运行与画面验收。模型输入与绑定证据完全一致时才复用结果；直接控制蒙皮网格变换的轨道会被拦截，以避免 Godot 导入丢失；整体移动使用共同的普通父节点，姿态使用加权骨骼，导入后的峰值变化仍需检查。旧资产不自动标为装配合格。 新工程可选 `adventure_animation_v1.gd` 将真实控制器状态绑定到明确指定的动作片段，处理混合、受击/死亡、暂停和复活，并拒绝驱动碰撞体的轨道；另提供按名称绑定骨骼挂点、只读鞋底探针和可配置攻击前摇；第三方真实骨骼角色已通过原生 UI 操作与存档结局恢复验证，仍不代表自然步态、IK 或自主成品验收。

探索类游戏可选 `GAME_ASSEMBLY_V1.md`：根据 manifest 连接已制作的玩家/区域场景、实际任务与出口节点、统一进度、原生 UI 和区域检查点。UI 色板绑定实际 ArtBible 文件，正式构建拒绝缺失映射与过期风格来源。三区域交互工程已通过系统键盘、Chromium 保存与刷新恢复；这是装配和来源验证，不能代替自主成品、战斗路线或参考美术验收。

可选装配方案现已连接击败任务前置/奖励容量锁，以及区域音乐和真实战斗/任务/结局音效。暂停恢复同曲时保留声部与播放位置，并修复当前 Web 运行时的重复开曲问题。已验证一条工程战斗路线的浏览器输入与混音输出；敌人死亡美术、音画延迟和完整自主游戏仍须验收。

离线 [CC0 模型库](resources/free-models/LICENSES.md) 提供 14 个 Kenney 场景素材，附预览、尺寸与来源摘要。Noobi 可通过专用工具列出并导入风格合适的素材，保留第三方来源；它们不自带游戏碰撞，也不替代用户要求的特色角色。
...
相机组件还可淡化被明确标记的前景墙，检查玩家、附近敌人的身体与脚部，以及配置好的危险区域边缘。遮挡解除后恢复各实例原材质，碰撞保持不变。支持标准/ORM 材质；自定义着色器和叠加材质会报告需要适配，不会被擅自替换。这是有采样上限的可读性辅助，不能代替所有镜头角度的实际验收。

多区域游戏可使用 `PROGRESSION_V1.md` 中的任务奖励、背包、能力、一次性开门消耗与存档状态组件。构建会检查可选的 `data/progression.json`，发现循环前置、不可达内容和钥匙消耗造成的卡关；状态探索超限时明确未验证。真实目标、几何路线及完整试玩仍须另外通过。
...
默认使用内置 CC0 素材库：2 首背景音乐、16 个音效，离线选取并导入，不调用音频 API。素材保留作者、来源和授权信息。这是现成素材选取，暂不提供定制配乐或语音；需要云端生成时，可在“设置 → 媒体 API”手动切换音频来源并保存。参见[素材来源与授权](resources/free-audio/LICENSES.md)。新 Godot 工程还提供音乐循环、双声部切换渐变、有限音效池与暂停/重开清理组件，复用游戏 UI 的分轨总线。原生混音及 13 步 Chromium 实际输入/UI 音频检查通过；播放器使用 Stream 混音避开 Web Sample 路由无声。Web 延迟、其他浏览器、生成成品接入与实际试听仍待验收。见[运行组件证据](docs/development-plan/reports/07-audio-runtime.md)。
...
参考对照现可并排查看冻结参考与宿主校验的游戏截图，分别保存五维证据说明。长测要求角色活动范围和状态断言，掉出关卡但仍渲染也会判失败；完整玩法与真人验收另行进行。

资源耗尽现在会暂停新的代码修复回合并保留剩余预算；先检查宿主内存/磁盘与游戏资源分配后再继续，错误文本本身不代表已定位根因。

模型远程压缩和认证请求故障会与游戏质量问题分开诊断；请求发送失败不等于登录失效。此类故障会停止新的游戏代码修复，不会据此自动重放请求、退回已消耗次数或改变累计执行预算。

macOS 工程导出检查已能打包固定 UI 夹具、校验原生签名和标题画面，并记录真实系统键盘流程及新进程存档恢复；生产导出按钮、干净设备与自主生成成品仍分别验收。

在 Mac 上，Godot 作品的版本历史现可点击 **导出 macOS 游戏包**，从冻结源码生成独立原生候选包，附本地签名、文件校验和许可说明，保留原工程。原生玩法与干净设备验收不会沿用 Web 检查结论。
