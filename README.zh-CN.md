...
新工程还提供 `MODEL_ASSETS_V2.md`：统一风格规格以及尺寸、枢轴/挂点、碰撞意图和动画要求。宿主独立重载 GLB 后测量并检查采样顶点与加权蒙皮是否实际变化，空节点和无权重骨骼不算有效动作；真实碰撞、朝向、风格和游戏内动画仍需运行与画面验收。模型输入与绑定证据完全一致时才复用结果；直接控制蒙皮网格变换的轨道会被拦截，以避免 Godot 导入丢失；整体移动使用共同的普通父节点，姿态使用加权骨骼，导入后的峰值变化仍需检查。旧资产不自动标为装配合格。
