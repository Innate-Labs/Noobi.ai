...
export interface GenerateGDDParams {
  ...
  /**
   * Game archetype (REQUIRED) - determined by classify_game_type tool in Phase 1
   */
...
> {
  ...
  getDescription(): string {
    return `为 ${this.params.archetype} 生成中文技术 GDD。`;
  }
...
<system-reminder>
GDD 已生成，游戏类型：**${this.params.archetype}**
...
- 仅 platformer 或 top_down/tilemap 使用 Section 4 的 ASCII 地图调用 \`generate_tilemap\`；arena、ui_heavy、tower_defense、grid_logic 使用代码数据
...
- 编码前使用绝对路径阅读 Module Manual \`${path.join(docsRoot, 'modules', this.params.archetype, `${this.params.archetype}.md`)}\` 及所有目标模板源码
...
不要停止，现在继续 Phase 3。
  ...
  private async buildSystemPrompt(): Promise<string> {
    ...
    let prompt = `# 中文技术游戏设计文档生成器

你是一名游戏设计工程师。请生成一份**中文 Technical Game Design Document**；每个 Section 都必须直接映射到后续工具输入或代码文件，不能停留在概念描述。
...
**核心规则：**
1. **忠实需求**：满足用户明确要求，不增加未要求的功能。
2. **Config-First**：数值写入 \`gameConfig.json\`，使用 \`{ "value": X }\` 包装。
3. **零重造基础系统**：只使用 template_api.md 中已有 Behavior 与 Hook。
4. **Hook 完整性**：引用的每个 Hook 必须真实存在于 template_api.md；不存在的 Hook 会导致编译失败。
5. **增量配置**：Section 2 只输出游戏专用字段，后续合并到已有配置；不得覆盖 \`screenSize\`、\`debugConfig\`、\`renderConfig\`。
...
根据上方 GDD Core Rules 生成包含 **6 个 Section**（Section 0–5）的中文技术 GDD。所有关键参数、文件、场景 key、素材 key 和数值必须明确，不能让代码 Agent 猜测。
...
> {
  ...
  constructor(
    ...
    super(
      ...
      `为指定游戏类型生成中文技术 GDD。必须在 classify_game_type 与脚手架复制完成后调用；工具会动态加载对应类型的设计规则。`,
      Kind.Think,
      {
        type: 'object',
        properties: {
          ...
          archetype: {
            type: 'string',
            description: 'classify_game_type 返回的游戏类型。',
            enum: [
              'platformer',
              'top_down',
              'grid_logic',
  ...
  protected override validateToolParamValues(
    ...
    if (!params.archetype) {
      return 'archetype 为必填项，请先运行 classify_game_type。';
    }
