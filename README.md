...
New workspaces also include `MODEL_ASSETS_V2.md`: a shared art bible and explicit scale, pivot/socket, collision-intent and animation requirements. The host measures independently reloaded GLBs and checks sampled vertex motion and weighted skin deformation (empty nodes and unused bones do not qualify); actual collision, facing, style and in-game animation still need runtime/visual review. Identical model inputs reuse output only while their bound files and evidence remain intact. Direct transform tracks on skinned mesh nodes are rejected because Godot can drop them; animate a shared ordinary parent or weighted bones instead. Import resampling still requires in-engine checks. Legacy assets remain assembly-unverified. New workspaces offer an opt-in `adventure_animation_v1.gd` adapter that binds controller states to explicit clips, blends transitions, handles damage/death, pause and revival, and rejects physics-targeting tracks. Named bone sockets, read-only sole probes and configurable attack windup are also available. A third-party skeletal character has passed native UI input and ending-save restoration checks; this does not certify natural gait, IK or autonomous finished games.
