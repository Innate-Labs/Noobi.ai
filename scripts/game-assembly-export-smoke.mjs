...
const combat=process.argv.includes('--combat-audio');
const fixture=JSON.parse(await readFile('.noobi-private/stage-08/'+(combat?'combat-audio':'assembly')+'-latest.json','utf8'));
const out=resolve('.noobi-private/stage-08/'+(combat?'combat-audio-export':'assembly-export'),new Date().toISOString().replaceAll(':','-'));
...
app.whenReady().then(async()=>{try{
 ...
 for(const name of ['data','scenes','scripts','.noobi',...(combat?['assets']:[])]) await cp(join(fixture.out,name),join(project.root,name),{recursive:true,filter:path=>!path.endsWith('.import')&&!path.endsWith('.uid')});
 for(const name of ['project.godot','main.tscn'])await copyFile(join(fixture.out,name),join(project.root,name));
 // Remove the engineering auto-run from both the derived fixture and its optional base harness.
 for(const name of ['test.gd',...(combat?['base-test.gd']:[])]){
  const source=await readFile(join(project.root,'scripts',name),'utf8');
  await writeFile(join(project.root,'scripts',name),source.replace('    if not OS.get_cmdline_user_args().has("--noobi-assembly-demo"): call_deferred("run")','    # Interactive export: no automated test runner.'));
 }
 ...
 await writeFile('.noobi-private/stage-08/'+(combat?'combat-audio-export':'assembly-export')+'-latest.json',JSON.stringify(result,null,2));
