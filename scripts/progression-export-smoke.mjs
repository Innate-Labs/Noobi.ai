...
const out=resolve('.noobi-private/stage-08/export',new Date().toISOString().replaceAll(':','-'));app.setPath('userData',join(out,'electron'));app.on('window-all-closed',()=>{});
...
func _initialize():
    ...
    print("PROGRESSION_EXPORTED_JSON_OK")
    quit()
`);
...
const result=await promisify(execFile)(status.tool.binaryPath,['--headless','--path',build.root,'--main-pack',join(build.root,'build/web/index.pck'),'--script','res://verify_pack.gd'],{timeout:30000,maxBuffer:1024*1024});assert.ok(result.stdout.includes('PROGRESSION_EXPORTED_JSON_OK'),result.stderr);assert.ok(!result.stderr.includes('ERROR'),result.stderr);
await writeFile(join(out,'results.json'),JSON.stringify({build:build.record,report,output:result.stdout,passed:true},null,2));console.log('PROGRESSION_EXPORT_OK',out,build.record.buildId);app.exit(0);
