...
const root = mkdtempSync(join(tmpdir(), 'noobi-budget-ui-'));
...
async function run() {
  let window;
  try {
    const evidence = resolve('.noobi-private/platform-budget'); await mkdir(evidence, { recursive: true });
    ...
    for (let i = 0; i < 6; i++) await store.reserve(first.session, 'reconnects');
    ...
    let grantRequests = 0;
    ipcMain.handle('noobi:project:extend-budget', async (_event, input) => {
      grantRequests++; await pause(100);
      const result = await store.extendBudget(input.projectId, input.planRunId, input.revision);
      window.webContents.send('noobi:event:production-progress', result); return result;
    });
    ...
    // Exhausted connection allowance survives the store/renderer restart.
    await assert.rejects(store.reserve(resumed.session, 'reconnects'), /预算用尽/);
    const blocked = await store.finish(resumed.session, 'failed', '执行预算用尽：网络重连已使用 6/6 次。最近连接错误：TLS handshake failed');
    window.webContents.send('noobi:event:production-progress', blocked); await pause(100);
    assert.match(await text(), /执行预算用尽/); assert.match(await text(), /TLS handshake/);
    window.setSize(960, 680); await pause(100);
    await window.webContents.executeJavaScript(`document.querySelector('.production-budget').open=true;document.querySelector('.production-budget button').scrollIntoView({block:'center',behavior:'instant'})`);
    await pause(150);
    await writeFile(join(evidence, 'budget-blocked.png'), (await window.webContents.capturePage()).toPNG());
    await window.webContents.executeJavaScript(`const button=document.querySelector('.production-budget button');button.click();button.click();button.click()`);
    await pause(250);
    assert.equal(grantRequests, 1);
    const extended = await store.read('fixture');
    assert.equal(extended.budget.used.reconnects, 6); assert.equal(extended.budget.limits.reconnects, 9);
    assert.equal(extended.budget.grants.length, 1); assert.equal(extended.status, 'failed'); assert.equal(extended.attempts.length, 2);
    assert.match(await text(), /已手动增加预算 1 次/);
    await writeFile(join(evidence, 'budget-extended.png'), (await window.webContents.capturePage()).toPNG());
    store = new ProductionRunStore(file); await store.init();
    await window.reload(); await pause(400);
    await window.webContents.executeJavaScript(`document.querySelector('.production-budget').open=true`);
    assert.match(await text(), /网络重连：6 \/ 9 次/);
    window.setSize(390, 680); await pause(200);
    assert.equal(await window.webContents.executeJavaScript('document.documentElement.scrollWidth <= innerWidth'), true);
    await window.webContents.executeJavaScript(`document.querySelector('.production-budget button').scrollIntoView({block:'center',behavior:'instant'})`);
    await pause(150);
    await writeFile(join(evidence, 'budget-narrow.png'), (await window.webContents.capturePage()).toPNG());
    await writeFile(join(evidence, 'ui-result.json'), JSON.stringify({ passed: true, realRendererAndIpc: true,
      persistedRestart: true, restoredHistory: true, budgetPersistence: true, tripleClickSingleGrant: true, noAutoStart: true, lateEventsRejected: true, narrowLayout: true,
      modelCalls: 0, userGameWrites: 0 }, null, 2));
    console.log('PRODUCTION_BUDGET_UI_OK');
